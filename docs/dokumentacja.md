...
### Metoda `_build_coarse_level_data`

```python
def _build_coarse_level_data(self, grid_size: Optional[float] = None) -> Tuple[Dict, Dict, Dict]:
```

**Parametry:**
- `grid_size`: Rozmiar komórek siatki dla rozdzielczości zgrubnej (domyślnie: 2 * promień).

**Opis:**
Buduje struktury danych na poziomie zgrubnym dla przycinania wielorozdzielczościowego. Metoda:
1. Dzieli przestrzeń na komórki siatki o zadanym rozmiarze.
2. Grupuje instancje każdego typu w komórki, tworząc "super-instancje" zgrubne.
3. Dla każdej instancji zgrubnej przechowuje:
   - Współrzędne komórki siatki
   - Zbiór identyfikatorów instancji dokładnych zawartych w komórce
   - Liczbę instancji dokładnych
4. Buduje relacje sąsiedztwa na poziomie zgrubnym:
   - Dwie instancje zgrubne są sąsiadami, jeśli dowolne ich instancje dokładne są sąsiadami
   - Wykorzystuje wcześniej obliczone relacje sąsiedztwa dokładnego
5. Tworzy mapowanie z instancji dokładnych do ich instancji zgrubnych.

### Metoda `_find_coarse_pattern_instances`

```python
def _find_coarse_pattern_instances(self, pattern_types: Tuple[str, ...], 
                                  coarse_instances: Dict, 
                                  coarse_neighbors: Dict) -> List[Tuple[Tuple[str, str], ...]]:
```

**Parametry:**
- `pattern_types`: Krotka typów cech tworzących wzorzec.
- `coarse_instances`: Słownik instancji zgrubnych według typu.
- `coarse_neighbors`: Słownik relacji sąsiedztwa zgrubnego.

**Opis:**
Znajduje wszystkie instancje wzorca na poziomie zgrubnym. Algorytm działa analogicznie do znajdowania instancji dokładnych:
1. Rozpoczyna od wszystkich instancji zgrubnych pierwszego typu we wzorcu.
2. Iteracyjnie rozszerza częściowe instancje wzorca:
   - Dla każdej częściowej instancji znajduje kandydatów następnego typu
   - Kandydatem jest instancja zgrubna, która sąsiaduje ze wszystkimi elementami częściowej instancji
   - Wykorzystuje przecięcie zbiorów sąsiadów do efektywnego znajdowania wspólnych sąsiadów
3. Jeśli na dowolnym etapie nie ma kandydatów do rozszerzenia, wzorzec nie ma instancji zgrubnych.
4. Zwraca listę kompletnych instancji wzorca na poziomie zgrubnym.

### Metoda `_calculate_coarse_participation_index`

```python
def _calculate_coarse_participation_index(self, pattern: Pattern,
                                        coarse_pattern_instances: List,
                                        coarse_instances: Dict) -> Tuple[float, Dict[str, float]]:
```

**Parametry:**
- `pattern`: Typy cech tworzące wzorzec do oceny.
- `coarse_pattern_instances`: Lista instancji wzorca na poziomie zgrubnym.
- `coarse_instances`: Słownik instancji zgrubnych z informacjami o instancjach dokładnych.

**Opis:**
Oblicza wskaźnik uczestnictwa na poziomie zgrubnym, który stanowi górne ograniczenie wskaźnika dokładnego:
1. Dla każdej instancji wzorca zgrubnego:
   - Pobiera wszystkie instancje dokładne zawarte w uczestniczących instancjach zgrubnych
   - Dodaje je do zbioru uczestniczących instancji dla każdego typu
2. Oblicza współczynniki uczestnictwa dla każdego typu:
   - Dzieli liczbę uczestniczących instancji dokładnych przez całkowitą liczbę instancji typu
3. Oblicza wskaźnik uczestnictwa jako minimum ze współczynników.
4. Zwraca wskaźnik uczestnictwa oraz słownik współczynników.

Kluczowa właściwość: wskaźnik zgrubny nigdy nie niedoszacowuje wskaźnika dokładnego, ponieważ zlicza wszystkie instancje dokładne w uczestniczących komórkach.

### Metoda `_multi_resolution_pruning`
...
**Parametry:**
- `candidates`: Lista kandydatów na wzorce do przycinania.
- `k`: Rozmiar ocenianych wzorców.
- `grid_size`: Rozmiar komórek siatki (domyślnie: 2 * promień).
- `verbose`: Czy wyświetlać statystyki przycinania.

**Opis:**
Wykonuje przycinanie wielorozdzielczościowe kandydatów na wzorce:
1. Buduje struktury danych poziomu zgrubnego przy pierwszym wywołaniu.
2. Dla każdego kandydata:
   - Znajduje instancje wzorca na poziomie zgrubnym
   - Oblicza wskaźnik uczestnictwa zgrubnego
   - Jeśli wskaźnik zgrubny < próg, kandydat jest przycinany (wskaźnik dokładny też będzie < próg)
3. Wyświetla statystyki przycinania:
   - Liczbę ocenionych i przyciętych kandydatów
   - Stopień przycinania (procent przyciętych)
   - Czas wykonania
4. Zwraca listę kandydatów, które przeszły przycinanie zgrubne.
